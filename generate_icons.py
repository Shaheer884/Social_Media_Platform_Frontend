...
def generate_icons():
    ...
    for size in sizes:
        ...
        # 2. Maskable icon - Full bleed square icon (the mobile OS handles cropping dynamically)
        maskable_img = img.resize((size, size), Image.Resampling.LANCZOS).convert("RGBA")
        maskable_img.save(os.path.join(output_dir, f"maskable-icon-{size}x{size}.png"), "PNG")
        print(f"Generated full-bleed maskable icon: maskable-icon-{size}x{size}.png")
